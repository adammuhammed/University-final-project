        ...
        <form>
          <div class="paper">
            ...
            <div class="second-body">
                <div class="title">
                  <p class="pt1">الى / الجامعة التكنولوجية / قسم {{ $user->arabic_department }} / شعبة الدراسات العليا</p>
                  <p class="pt2">م / وثيقة تخرج</p>
                </div>
                <div class="write">
                  <p class="pw1">
                    <br>
                  نؤيد لكم بأن الخريج ({{ $user->arabic_name }}) المثبة صورته اعلاه قد تخرج من هذه الجامعة في {{ $final_attempt }} بتقدير ({{ $grade }}) وحصل على بكالوريوس علوم في {{ $user->arabic_department }} (هندسة الحاسبات وتكنولوجيا المعلومات سابقا) / {{ $user->arabic_branch }} بتاريخ {{ date('Y/m/d', strtotime($user->date_of_graduation)) }} للعام الدراسي {{ date('Y', strtotime($user->date_of_graduation. '+1 year')) }}-{{ date('Y', strtotime($user->date_of_graduation)) }} وبمعدل ({{ $total_avg }}) وتسلسله ({{ $rank }}) من مجموع ({{ $total_students }}) خريج على الفرع في الدراسة الصباحية علما ان معدل الطالب الاول على الدفعة للدورين الاول والثاني  وللدراستين الصباحية والمسائية هو ({{ $first_total_avg }}) وان مدة الدراسة هي اربع سنوات بعد الدراسة الثانوية.
                  <br>
                  <br>
                  <br>
                ...
                <div class="signture">
                  <div class="s1">
                  <p class="ps23">{{ $data['pou'] }}</p>
                  <p class="ps22">معاون رئيس القسم</p>
