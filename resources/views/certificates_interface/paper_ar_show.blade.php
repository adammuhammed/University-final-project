...
<div class="title">
  <p class="pin1">إلى / من يهمه الأمر</p>
  <p class="pin2">الموضوع / وثيقة تخرج</p>
</div>
<div class="write">
  <p class="pin3">
    نؤيد بأن {{ $user->arabic_name }} ({{ $user->arabic_country }} الجنسية) المثبتة صورته اعلاه قد تخرج من هذه الجامعة {{ $final_attempt }} بتقدير ({{ $grade }}) وحصل على درجة بكالوريوس علوم في {{ $user->arabic_department }}  / {{ $user->arabic_branch }} بتاريخ {{ date('Y/m/d', strtotime($user->date_of_graduation)) }} للعام الدراسي {{ date('Y', strtotime($user->date_of_graduation)) }}-{{ date('Y', strtotime($user->date_of_graduation. '+1 year')) }} علما ان مدة الدراسة هي اربع سنوات بعد الدراسة الثانوية.
  </p>
</div>
<div class="signtures">
  <div class="s1">
    <p class="ps11">{{ $data['hod'] }}</p>
    <p class="ps12">رئيس القسم</p>
  </div>
  <div class="s2">
    <p class="ps21">{{ $data['ur'] }}</p>
    <p class="ps22">مسجل الجامعة</p>
  </div>
  <div class="s3">
    <p class="ps31">{{ $data['pou'] }}</p>
    <p class="ps32">رئيس الجامعة</p>
  </div>
</div>
      </div>
